...
/* Donde la llave es un numero entero unicamente */
/* Se ejecuta desde el programa MCURB18.c */
		...
		if (posicion == -1)
		   *s = 1;              /* La llave esta en el arbol */
			...
			while (!pila_vacia (&pila) && terminar == 0) {
			  ...
			  if (a[p].cont == M) {
			     if (separar == 0) {
				romper (a, p, 0, &nuevo, x, &subir,separar);
				separar = 1;
			     }
			     else {
	...
	while (terminar == 0) {
	  if (a[r].cont < N && a[p].cont > N) {	/* CASO 7 */
		...
		terminar = 1;
		  ...
		  j = a[r].cont;
		  if (a[r].apunt [0] == 0)	/* Si es el ultimo nivel */
...
retira_b (CURSOR a[],int *raiz, int *VSAM, int x, int *s)
{
	int posicion, i, k, p, q, r, t, temp;
	LIFO1 pila;
	...
	*s = 1;
	...
	if (i > 0)
		i--;
	unir (a,raiz,q,r,p,i,pila,x,posicion);
}
